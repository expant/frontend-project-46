...
test.each(['json', 'yml', 'yaml'])('gendiff (%s) format', () => {
  const filepath1 = readFileSync(getFixturePath('file1.%s'), 'utf-8');
  const filepath2 = readFileSync(getFixturePath('file2.%s'), 'utf-8');
  
  const data1 = '%s' === 'json' ? JSON.parse(filepath1) : yaml.load(filepath1);
  const data2 = '%s' === 'json' ? JSON.parse(filepath2) : yaml.load(filepath2);

  expect(genDiff(data1, data2)).toBe()
